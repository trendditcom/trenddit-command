...
1. Read the Trenddit README and API Documentation.
2. Think of how to best present the features, capabilities, and benefits of Trenddit.
...
Setup the multi-page streamlit app in a new folder name trenddit-command. Change to this folder.
Now create the main script within trenddit-command folder.
...
# Trenddit API Documentation

Trenddit is a Python package that provides a command-line interface for interacting with various AI models and performing tasks such as text generation, image creation, and more. This documentation covers the API for using Trenddit within external projects.
...
Trenddit provides a command-line interface with various commands. Here's an overview of the available commands:

- `run`: Processes a selected markdown file and runs the code blocks within it.
- `audit`: Analyzes the usage of Trenddit over time and generates a report.
...
- `init`: Initializes Trenddit in the current directory.
- `config`: Edits the Trenddit configuration.
...
The `configure` module handles configuration management for Trenddit.

#### Functions:

```python
def load_config(section: Optional[str] = None) -> Dict[str, Any]:
    # Load the configuration from trenddit-command.yml

def has_vision_capability(model: str) -> bool:
    # Check if a model has vision capabilities

def save_config(config: Dict[str, Any]):
    # Save the configuration to trenddit-command.yml
...
The `images` module provides utility functions for handling images in Trenddit.
...
The `utils` module provides utility functions and decorators for the Trenddit package.
