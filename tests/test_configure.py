...
def test_load_config_no_file():
    with patch("os.path.exists", return_value=False):
        from trenddit_command.configure import load_config
...
def test_has_vision_capability():
    with patch("trenddit_command.configure.load_config", return_value=mock_config_data):
        from trenddit_command.configure import has_vision_capability
...
def test_save_config():
    with patch("builtins.open", mock_open()) as mocked_file:
        from trenddit_command.configure import save_config

        save_config(mock_config_data)
        mocked_file.assert_called_once_with("trenddit_command.yml", "w")
...
def test_edit_config_existing_key():
    ...
    with patch("trenddit_command.configure.load_config", return_value=mock_config_data), patch(
        "trenddit_command.configure.save_config"
    ) as mock_save_config:
        from trenddit_command.configure import edit_config
...
def test_edit_config_type_conversion():
    ...
    with patch("trenddit_command.configure.load_config", return_value=initial_config), patch(
        "trenddit_command.configure.save_config"
    ) as mock_save_config:
        from trenddit_command.configure import edit_config
...
def test_get_model_mapping():
    with patch("trenddit_command.configure.load_config", return_value=mock_config_data):
        from trenddit_command.configure import get_model_mapping
...
def test_resolve_model():
    with patch(
        "trenddit_command.configure.get_model_mapping",
        return_value=mock_config_data["model-mapping"],
    ):
        from trenddit_command.configure import resolve_model
